...
return require('packer').startup(function(use)
    ...
    use { 'kyazdani42/nvim-tree.lua', config = function() require('nvim-tree_conf') end } -- file explorer
    use { 'kyazdani42/nvim-web-devicons', config = function() require('nvim-web-devicons_conf') end } -- filetype icons
    use { 'mbbill/undotree', config = function() require('undotree_conf') end } -- multilevel undo explorer
    use 'christoomey/vim-tmux-navigator' -- easily move from between vim windows and tmux panes
    use { 'jpalardy/vim-slime', config = function() require('vim-slime_conf') end } -- send text to terminal
    use { 'chentoast/marks.nvim', config = function() require('marks').setup() end } -- better marks support
    ...
    use { 'kylechui/nvim-surround', config = function() require('nvim-surround').setup {} end } -- surround text objects with opening/closing symbols
    use 'wellle/targets.vim' -- extend default text objects
    use { 'ntpeters/vim-better-whitespace', config = function() require('vim-better-whitespace_conf') end } -- handle trailing whitespace
    use 'mg979/vim-visual-multi' -- multiple cursors
    use { 'junegunn/vim-easy-align', config = function() require('vim-easy-align_conf') end } -- easily align text
    use { 'nvim-lualine/lualine.nvim', requires = { 'kyazdani42/nvim-web-devicons', opt = true },
        config = function() require('lualine_conf') end } -- status line (bottom)
    use { 'akinsho/bufferline.nvim', tag = "v2.*", requires = 'kyazdani42/nvim-web-devicons',
        config = function() require('bufferline_conf') end } -- buffer line (top)
    ...
    use { 'ellisonleao/gruvbox.nvim', config = function() require('gruvbox_conf') end } -- colorscheme
    use { 'p00f/nvim-ts-rainbow', config = function() require('nvim-ts-rainbow_conf') end } -- change color on (), []... for better readability when on treesitter aware syntax
    ...
    use { 'hrsh7th/nvim-cmp', requires = { 'L3MON4D3/LuaSnip' }, config = function() require('cmp_conf') end } -- autocompletion engine
    ...
    use { 'L3MON4D3/LuaSnip', config = function() require('luasnip_conf') end } -- snippet engine
    use 'saadparwaiz1/cmp_luasnip' -- autocompletion (luasnip snippets)
    use 'rcarriga/cmp-dap' -- autocompletion (debugger)
    use { 'tpope/vim-fugitive', config = function() require('fugitive_conf') end } -- git handler
    use 'honza/vim-snippets' -- rich snippet collection
    use 'rafamadriz/friendly-snippets' -- rich snippet collection
    use { 'lewis6991/gitsigns.nvim', config = function() require('gitsigns_conf') end } -- git modifications explorer/handler
    ...
    use { 'fatih/vim-go', config = function() require('vim-go_conf') end } -- go filetype enhancements, commands and project handling
    ...
    use { 'nvim-telescope/telescope.nvim', config = function() require('telescope_conf') end } -- fuzzy finding anything anywhere
    use { 'nvim-treesitter/nvim-treesitter', config = function() require('treesitter_conf') end } -- enhanced syntax by treesitter
    ...
    use { 'neovim/nvim-lspconfig', config = function() require('lsp_conf') end,
        requires = { 'williamboman/nvim-lsp-installer' } } -- easily config neovim lsp
    use { 'williamboman/nvim-lsp-installer', config = function() require('nvim-lsp-installer').setup {} end } -- easily install/handle lsp servers directly from neovim
    use { 'lukas-reineke/indent-blankline.nvim', config = function() require('indent-blankline_conf') end } -- indent lines
    use { 'sindrets/diffview.nvim', requires = 'nvim-lua/plenary.nvim' } -- nice view for git diff
    use { 'windwp/nvim-autopairs', config = function() require('nvim-autopairs').setup {} end } -- autoclose pairs, (), []...
    use { 'numToStr/Comment.nvim', config = function() require('Comment').setup() end } -- syntax aware commenting
    use { 'glacambre/firenvim', run = function() vim.fn['firenvim#install'](0) end,
        config = function() require('firenvim_conf') end } -- embed neovim on the browser
    use { 'mfussenegger/nvim-dap', config = function() require('dap_conf') end } -- debugger engine
    use { 'mfussenegger/nvim-dap-python', config = function() require('dap-python_conf') end } -- dap python debugger
    use { 'rcarriga/nvim-dap-ui', requires = { 'mfussenegger/nvim-dap' }, config = function() require('dap-ui_conf') end } -- dap ui
    use { 'folke/trouble.nvim', config = function() require('trouble_conf') end } -- a pretty list for diagnostics
    use { 'akinsho/toggleterm.nvim', tag = '*', config = function() require('toggleterm_conf') end } -- neovim terminal handler
    ...
    use { 'toppair/peek.nvim', run = 'deno task --quiet build:fast', config = function() require('peek_conf') end } -- markdown preview plugin based on deno
    use { 'nat-418/boole.nvim', config = function() require('boole_conf') end } -- enhance C-A, C-X
    use { 'vifm/vifm.vim', config = function() require('vifm_conf') end } -- vifm file manager inside neovim
    use { 'folke/zen-mode.nvim', config = function() require('zen_mode_conf') end } -- zen mode, enhanced window zooming, neorg presentation friendly
